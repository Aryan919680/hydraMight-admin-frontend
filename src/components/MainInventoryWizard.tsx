...
export function MainInventoryWizard({
    ...
    useEffect(() => {
        if (editing || !form.sku.trim() || mode !== "single") {
            setDuplicate(null);
            return;
        }
        ...
        const timer = window.setTimeout(async () => {
            try {
                setCheckingSku(true);
                const response = await api.checkMainInventorySku(form.sku.trim());
                setDuplicate(response.exists ? response.data : null);
            } catch {
                setDuplicate(null);
            } finally {
    ...
    const validateDetails = () => {
        ...
        if (!/^[A-Z0-9][A-Z0-9._-]*$/i.test(form.sku.trim())) {
            next.sku = "Use letters, numbers, hyphens, dots or underscores only.";
        }
        if (!form.item_name.trim()) next.item_name = "Item name is required.";
        if (duplicate) next.sku = "This SKU already exists. Edit the existing inventory instead.";
        setErrors(next);
    ...
    const downloadSample = () => {
        const csv = [
            "sku,item_name,category,total_stock,reserved_stock,min_stock_level,remarks",
            ...sampleRows.map((row) => Object.values(row).join(",")),
        ].join("\n");
    ...
    const processBulk = async (validateOnly: boolean) => {
        if (!file) return;
        try {
            validateOnly ? setValidating(true) : setUploading(true);
            const response = await api.bulkUploadMainInventory(file, validateOnly);
            setBulkResult(response);
            if (!validateOnly) await onSaved();
        } finally {
            setValidating(false);
            setUploading(false);
        }
            ...
            <div className="space-y-6">
                ...
                {step === 1 && (
                    <section className="rounded-xl border p-6">
                        ...
                        <div className="grid gap-5 md:grid-cols-2">
                            <Field label="SKU code" required error={errors.sku}>
                                <div className="relative">
                                    <Input value={form.sku} disabled={editing} onChange={(e) => setField("sku", e.target.value.toUpperCase())} placeholder="e.g. HYD-FC-500ML" className={errors.sku ? "border-destructive" : ""} />
                                    {checkingSku && <Loader2 className="absolute right-3 top-3 h-4 w-4 animate-spin" />}
                                </div>
                            </Field>
                        ...
                        <DialogFooter className="mt-6"><Button onClick={next} disabled={checkingSku}>Continue</Button></DialogFooter>
                    </section>
                )}
                ...
                {step === 2 && (
                    <section className="rounded-xl border p-6">
                        ...
                        <DialogFooter className="mt-6 justify-between sm:justify-between"><Button variant="outline" onClick={() => setStep(1)}>Back</Button><Button onClick={next}>Review</Button></DialogFooter>
                    </section>
                )}
    ...
    function BulkFlow() {
        ...
        const hasErrors = Number(bulkResult?.failed || 0) > 0;
        return (
            <div className="space-y-5">
                ...
                <section className="rounded-xl border border-dashed p-6">
                    ...
                    <div className="mt-4 flex gap-3"><Button variant="outline" disabled={!file || validating || uploading} onClick={() => processBulk(true)}>{validating && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Validate CSV</Button><Button disabled={!file || uploading || validating || !bulkResult || hasErrors || bulkResult.validate_only === false} onClick={() => processBulk(false)}>{uploading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Upload className="mr-2 h-4 w-4" />}Import valid rows</Button></div>
                </section>
